...
<#import "components/atoms/hero.ftl" as hero>
...
>
  ...
  <#assign cardHeader>
    ...
    <#if !(auth?has_content && auth.showUsername() && !auth.showResetCredentials())>
      ...
      <#assign subtitleText = subtitle?is_markup_output?then(subtitle?markup_string, subtitle)>
      <#if subtitleText?trim?has_content>
        <p class="text-center text-sm text-secondary-800">
          ${subtitle}
        </p>
      </#if>
    <#else>
  ...
  <html<#if realm.internationalizationEnabled> lang="${locale.currentLanguageTag}"</#if>>
    ...
    <@body.kw>
      <@container.kw>
        <@card.kw content=cardContent footer=cardFooter header=cardHeader hero=cardHero />
        <@nav.kw>
          <#nested "nav">
